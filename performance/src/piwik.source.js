...
if (window.Piwik && typeof window.Piwik.PerformanceTrace !== 'object') {
    Piwik.PerformanceTrace = (function () {
        ...
        this.run = function () {
            ...
            _.getGeoPosition(function (data) {
                ...
                Piwik.DOM.onLoad(function () {
                    ...
                    options.dlt = perfomanceData.lookupDomainTime;
                    options.ct = perfomanceData.connectTime;
                    options.rrt = perfomanceData.requestTime;
                    options.cet = perfomanceData.domContentLoadEventTime;
                    //dom结构加载时间（包含内js/css等下载时间）
                    options.drt = perfomanceData.domReadyTime;

                    options.wst = perfomanceData.firstPaintTime;
                    options.dt = perfomanceData.domLoadedTime;
                    options.at = perfomanceData.loadTime;
        ...
        this.getPerformance = function (global) {
            ...
            return global.performance || global.webkitPerformance || global.msPerformance || global.mozPerformance;
        };
        this.getPageTiming = function () {
                ...
                if (isArray(obj)) {
                    obj.forEach(function (v, k) {
                        if (/\[\]$/.test(k)) {
                            ...
                            buildParams(prefix + '[' + (isObject(v) ? k : '') + ']', v, add);
                        }
                    });
        ...
        this.getResourceTiming = function(){
            ...
            return allResourceTiming.map(function(item) {
                return {
                    ...
                    rn: item.name,
                    //解压资源大小
                    ds: item.decodedBodySize,
                    //压缩资源大小
                    es: item.encodedBodySize,
                    //重定向
                    rdt: _.toDecimal(item.redirectEnd - item.redirectStart),
                    //域名解析时间
                    dlt: _.toDecimal(item.domainLookupEnd - item.domainLookupStart),
                    //tcp连接时间
                    ct: _.toDecimal(item.connectEnd - item.connectStart),
                    //请求响应时间
                    rrt: _.toDecimal((item.responseEnd || item.responseStart) - item.requestStart),
                    //页面加载完成
                    at: _.toDecimal(item.duration),
                    // resource type (include xmlhttprequest、iframe)
                    it: item.initiatorType,
                    // queueing and  stalled times
                    // qs: _.toDecimal((item.domainLookupStart || item.domainLookupEnd || item.connectStart) - item.startTime)
                }
            });
        };
        ...
        this.getGeoPosition = function (fn, options) {
